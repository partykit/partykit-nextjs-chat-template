export default function RoomMessage(props: { message, isMe }) {
    ...
    if (message.from.id === "system") {
        return (
            <li className="text-stone-400 flex flex-col justify-center items-center gap-1">
                ...
                <span className="text-xs">{ new Date(message.at).toLocaleTimeString() }</span>
            </li>
        )
    }
    else {
        return (
            <li className={`flex flex-col justify-center gap-1 ${isMe ? "items-end" : "items-start"}`}>
                ...
                <span className="text-xs text-stone-400">{ new Date(message.at).toLocaleTimeString() }</span>
            </li>
        )
    }
}